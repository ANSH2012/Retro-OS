...
async function fetchWithValidatedRedirects(targetUrl, controller) {
  ...
  for (let hop = 0; hop < 5; hop += 1) {
    const response = await fetch(currentUrl, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; RetroOSBrowser/1.0; +guns.lol/4nsh)",
        Accept:
...
export default async function handler(req, res) {
  ...
  if (rawUrl.toLowerCase() === "ansh") {
    return res.redirect(302, "https://guns.lol/4nsh");
  }
  ...
  if (isExternalOnlyHostname(targetUrl.hostname)) {
    // For known external-only websites, redirect directly to the real URL instead of rendering it inside RetroOS.
