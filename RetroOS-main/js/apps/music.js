...
function switchToSpotify() {
  ...
  // Destroy existing controller to reset to beginning
  if (embedController) {
...
function createMusicPlayer() {
  ...
  const options = {
    uri: "spotify:playlist:4DcrOCGmjlok3eMJsn4wH5",
    width: "100%",
    height: "100%",
    theme: "dark",
  };
...
/**
 * Resets the music state to initial state for fresh start
 */
function resetMusicState() {
  destroyMusicPlayer();
  // Reset local background music
  if (bgmAudio) {
    bgmAudio.currentTime = 0;
    bgmAudio.pause();
  }
