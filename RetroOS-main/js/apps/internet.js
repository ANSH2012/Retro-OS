...
function normalizeInternetTarget(input) {
  ...
  // Normalize common spaced URL input such as "youtube .com" to "youtube.com"
  if (value.includes(" ")) {
    const noSpaces = value.replace(/\s+/g, "");
    if (/^[^\s]+\.[^\s]+$/.test(noSpaces)) {
      value = noSpaces;
    }
  }

  // Custom redirect: searching "ansh" goes directly to your target URL
  if (value.toLowerCase() === "ansh") {
    return {
      mode: "search",
      displayValue: "https://guns.lol/4nsh",
      targetUrl: "https://guns.lol/4nsh",
      status: "Opening custom Ansh redirect...",
    };
  }
...
function navigateInternet(inputValue, options = {}) {
  ...
  if (
    ...
    window.open(nextTargetUrl, "_blank", "noopener,noreferrer");
    setInternetStatus(`Opened ${new URL(nextTargetUrl).hostname} in a new tab.`);
    return;
  }
